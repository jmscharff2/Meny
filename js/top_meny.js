...
var Top_Meny = {
	...
	create: function( options ) {
		return (function(){
			...
			// options
			Top_Meny.extend( config, options );
			...
			function setupCover() {
				...
				if( supports3DTransforms ) {
					dom.cover.style[ Top_Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
			...
			function setupMenu() {
				...
				if( supports3DTransforms ) {
					style[ Top_Meny.prefix( 'transform' ) ] = menuTransformClosed;
					style[ Top_Meny.prefix( 'transformOrigin' ) ] = menuTransformOrigin;
					style[ Top_Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					Top_Meny.extend( style, menuStyleClosed );
				}
			}
			...
			function setupContents() {
				...
				if( supports3DTransforms ) {
					style[ Top_Meny.prefix( 'transform' ) ] = contentsTransformClosed;
					style[ Top_Meny.prefix( 'transformOrigin' ) ] = contentsTransformOrigin;
					style[ Top_Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
				}
				else {
					style.position = style.position.match( /relative|absolute|fixed/gi ) ? style.position : 'relative';
					Top_Meny.extend( style, contentsStyleClosed );
				}
			}
			...
			function bindEvents() {
				if( 'ontouchstart' in window ) {
					Top_Meny.bindEvent( document, 'touchstart', onTouchStart );
					Top_Meny.bindEvent( document, 'touchend', onTouchEnd );
				}
				else {
					Top_Meny.bindEvent( document, 'mousedown', onMouseDown );
					Top_Meny.bindEvent( document, 'mouseup', onMouseUp );
					Top_Meny.bindEvent( document, 'mousemove', onMouseMove );
				}
			}
			...
			function open() {
				if( !isOpen ) {
					...
					if( supports3DTransforms ) {
						...
						dom.contents.style[ Top_Meny.prefix( 'transform' ) ] = contentsTransformOpened;
						dom.menu.style[ Top_Meny.prefix( 'transform' ) ] = menuTransformOpened;
					}
					// ...fall back on JS animation
					else {
						menuAnimation && menuAnimation.stop();
						menuAnimation = Top_Meny.animate( dom.menu, menuStyleOpened, 500 );
						contentsAnimation && contentsAnimation.stop();
						contentsAnimation = Top_Meny.animate( dom.contents, contentsStyleOpened, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = Top_Meny.animate( dom.cover, { opacity: 1 }, 500 );
					}

					Top_Meny.dispatchEvent( dom.menu, 'open' );
				}
			}
			...
			function close() {
				if( isOpen ) {
					...
					if( supports3DTransforms ) {
						...
						dom.contents.style[ Top_Meny.prefix( 'transform' ) ] = contentsTransformClosed;
						dom.menu.style[ Top_Meny.prefix( 'transform' ) ] = menuTransformClosed;
					}
					// ...fall back on JS animation
					else {
						menuAnimation && menuAnimation.stop();
						menuAnimation = Top_Meny.animate( dom.menu, menuStyleClosed, 500 );
						contentsAnimation && contentsAnimation.stop();
						contentsAnimation = Top_Meny.animate( dom.contents, contentsStyleClosed, 500 );
						coverAnimation && coverAnimation.stop();
						coverAnimation = Top_Meny.animate( dom.cover, { opacity: 0 }, 500, function() { dom.cover.style.visibility = 'hidden'; } );
					}

					Top_Meny.dispatchEvent( dom.menu, 'close' );
				}
			}
			...
			function onTouchStart( event ) {
				...
				Top_Meny.bindEvent( document, 'touchmove', onTouchMove );
			}
			...
			function onTouchEnd( event ) {
				Top_Meny.unbindEvent( document, 'touchmove', onTouchMove );
			...
			return {
				...
				addEventListener: function( type, listener ) {
					dom.menu && Top_Meny.bindEvent( dom.menu, type, listener );
				},
				removeEventListener: function( type, listener ) {
					dom.menu && Top_Meny.unbindEvent( dom.menu, type, listener );
				}
			};
	...
	dispatchEvent: function( element, type, properties ) {
		if( element ) {
			...
			Top_Meny.extend( event, properties );
